...
public final class SiteEditService {
     ...
     */
    private static String pageseformat = "<!-- Page Settings: {" + "\"PageID\":\"%1$s\"," +              // page tcm uri
            "\"PageModified\":\"%2$tFT%2$tH:%2$tM:%2$tS\"," +        // page modified date (2012-04-05T17:33:02)
            "\"PageTemplateID\":\"%3$s\"," +      // page template tcm uri
            "\"PageTemplateModified\":\"%4$tFT%4$tH:%4$tM:%4$tS\"" + // page template modified date
            // (2012-04-05T17:33:02)
            "} -->" + "<script type=\"text/javascript\" language=\"javascript\" defer=\"defer\" src=\"%5$s\" " +
    ...
    public static String generateSiteEditPageTag(Page page) {
        try {
            ...
            if (settings.isEnabled() || settings.hasPubSE(uri.getPublicationId())) {
                ...
                return String.format(pageseformat, page.getId(), page.getRevisionDate(), page.getPageTemplate().getId
     ...
     */
    private static String componentseformat = "<!-- Start Component Presentation: {" + "\"ComponentID\" : \"%1$s\", "
            +               // component tcm uri
            "\"ComponentModified\" : \"%2$tFT%2$tH:%2$tM:%2$tS\", " +         // component modified date
            // (2012-04-05T17:33:02)
            "\"ComponentTemplateID\" : \"%3$s\", " +       // component template id
            "\"ComponentTemplateModified\" : \"%4$tFT%4$tH:%4$tM:%4$tS\", " + // component template modified date
            // (2012-04-05T17:33:02)
            "\"IsRepositoryPublished\" : %5$b" +           // is repository published (true if dynamic component
            // template, false otherwise)
//    "%6$b" +                                       // is query based (true for a broker queried dcp, omit if
// component presentation is embedded on a page)
            "} -->";
     ...
     */
    public static String generateSiteEditComponentTag(ComponentPresentation cp, int orderOnPage, String region,
                                                      ServletRequest req) throws Exception {
        if (!isSiteEditEnabled(cp.getComponent().getId(), req)) {
            return "";
        }

        return String.format(componentseformat, cp.getComponent().getId(), cp.getComponent().getRevisionDate(), cp
                .getComponentTemplate().getId(), cp.getComponentTemplate().getRevisionDate(), cp.isDynamic());
    }


    private static String linkedcomponentseformat = "<!-- Start Component Presentation: {" + "\"ID\" :  " +
            "\"tcm:%1$d-%2$d\", " +                            // pub uri + component tcm uri
            ...
            "\"IsRepositoryPublished\" : %5$b, " +                    // is repository published (true if dynamic
            // component template, false otherwise)
            "\"IsQueryBased\" : %6$b" +                                 // is query based (true for a broker queried
            // dcp, omit if component presentation is embedded on a page)
            "} -->";

...
public class EHCacheProvider implements PayloadCacheProvider, CacheInvalidator,
	 ...
	 * @param cacheElement
	 *            CacheElement a wrapper around the actual value to store in
	 *            cache
	 */
	@Override
	 ...
	 * @param cacheElement
	 *            Object the actual value to store in cache
	 * @param dependingPublicationId
	 *            int representing the Publication id of the Tridion item the
	 *            cacheItem depends on
	 * @param dependingItemId
	 *            int representing the Item id of the Tridion item the cacheItem
	 *            depends on
	 */
	@Override
	public <T> void storeInItemCache(String key, CacheElement<T> cacheElement,
	...
	public void invalidate(final String key) {
		...
		if (dependencyElement != null) {
			...
			if (cacheSet != null) {
				for (Iterator<String> iterator = cacheSet.iterator(); iterator
						.hasNext();) {
			...
			LOG.info(
					"Attempting to expire key {} but not found in dependency cache",
					dependencyKey);
		}
	}
	...
	private void addDependency(String cacheKey, String dependencyKey) {
		...
		Element dependencyElement = dependencyCache.get(dependencyKey);
		if (dependencyElement != null) {
			cacheSet = ((CacheElement<ConcurrentSkipListSet<String>>) dependencyElement
					.getObjectValue()).getPayload();
			setNotExpired(dependencyElement);
		}
		if (cacheSet == null) {
			LOG.debug("Add empty dependencies for key: {} to cache",
					dependencyKey);
			cacheSet = new ConcurrentSkipListSet<>();
			CacheElementImpl<ConcurrentSkipListSet<String>> cacheElement = new CacheElementImpl<ConcurrentSkipListSet<String>>(
					cacheSet);
		...
		LOG.debug("Add dependency from key: {} to key: {}", dependencyKey,
				cacheKey);
	...
	private void expireElement(Element element, int adjustTTL) {
		...
		int timeToLive = lastAccessTime == 0 ? expiredTTL
				: (int) (lastAccessTime - creationTime) / 1000 + expiredTTL;
	...
	private void setNotExpired(Element dependentElement) {
		...
		CacheElement cacheElement = (CacheElement) dependentElement
				.getObjectValue();
		if (cacheElement.isExpired()) {
			cacheElement.setExpired(false);
	...
	private void updateTTL(Element dependentElement) {
		...
		int timeToLive = lastAccessTime == 0 ? cacheDependencyTTL
				: (int) (lastAccessTime - creationTime) / 1000
						+ cacheDependencyTTL;
